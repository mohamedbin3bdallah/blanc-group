...
function insertSale($saleid,$product,$titleen,$titlear,$price,$quantity,$total,$color,$size,$phone,$address,$ipaddress)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$stmt = $dbh->prepare("insert into `sales` (`saleid`,`product`,`titleen`,`titlear`,`price`,`quantity`,`total`,`color`,`size`,`phone`,`address`,`ipaddress`,`dtime`) values ('$saleid','$product','$titleen','$titlear','$price','$quantity','$total','$color','$size','$phone','$address','$ipaddress','')");
	$stmt->execute();
	include("libs/closedb.php");
}
