...
function getProductByID($id,$lang)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select id,title{$lang} as title,description{$lang} as description,categories,sizes,colors,oldprice,price,quantity,views from products where id = '$id'");
	$allrows = array();	
	if(!empty($result))
	{
		$row = $result->fetch();
...
function getRelatedProducts($lang,$id,$category,$productnumber)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select id,title{$lang} as title,oldprice,price from products where quantity > 0 and id != '$id' LIMIT 0,$productnumber");
	$allrows = array();	
	if(!empty($result))
	{
		for($i=0; $row = $result->fetch(); $i++)
