...
function getnoOfProducts($q,$category,$size,$color)
{
	include("libs/config.php");
	include("libs/opendb.php");	
	$result = $dbh->query("select count(*) as count from products where quantity > 0 and (titleen like '%$q%' or titlear like '%$q%') and categories like '%$category%' and sizes like '%$size%' and colors like '%$color%'");
	$row = $result->fetch();
	include("libs/closedb.php");
	return $row['count'];
}

function getProducts($lang,$q,$category,$size,$color,$startResults,$resultsPerPage)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select id,title{$lang} as title,categories,oldprice,sizes,colors,price from products where quantity > 0 and (titleen like '%$q%' or titlear like '%$q%') and categories like '%$category%' and sizes like '%$size%' and colors like '%$color%' LIMIT $startResults, $resultsPerPage");
	$allrows = array();	
	if(!empty($result))
	{
		for($i=0; $row = $result->fetch(); $i++)
		{
			...
			$allrows[$i]['sizes'] = $row['sizes'];
			$allrows[$i]['colors'] = $row['colors'];
...
function getAllProducts($lang,$startResults,$resultsPerPage)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select id,title{$lang} as title,categories,oldprice,sizes,colors,price from products where quantity > 0 LIMIT $startResults, $resultsPerPage");
	$allrows = array();	
	if(!empty($result))
	{
		for($i=0; $row = $result->fetch(); $i++)
