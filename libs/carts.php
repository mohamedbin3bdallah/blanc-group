...
function countIpSale($ipaddress)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select count(*) as count from sales where ipaddress = '$ipaddress' and delivered = 0");
	$row = $result->fetch();
	include("libs/closedb.php");
...
function getCarts($lang,$ipaddress,$startResults,$resultsPerPage)
{
	include("libs/config.php");
	include("libs/opendb.php");
	$result = $dbh->query("select id,product,title{$lang} as title,price,quantity,color,size from cart where ipaddress = '$ipaddress' LIMIT $startResults, $resultsPerPage");
	$allrows = array();	
	if(!empty($result))
	{
		for($i=0; $row = $result->fetch(); $i++)
