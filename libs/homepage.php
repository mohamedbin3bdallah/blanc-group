...
function getValues()
{
	...
	$result = $dbh->query("select * from ourvals");
	$allrows = array();	
	if(!empty($result))
	{
...
function updateValue($id,$titleen,$titlear,$descriptionen,$descriptionar,$image)
{
	include("../libs/config.php");
	include("../libs/opendb.php");
	$stmt = $dbh->prepare("update ourvals set titleen = '$titleen',titlear = '$titlear',descriptionen = '$descriptionen',descriptionar = '$descriptionar',image = '$image' where id = '$id'");
	$stmt->execute();	
	include("../libs/closedb.php");
}
