							...
							<table class="table checkouttable">
								...
								<?php for($i=0;$i<sizeof($carts);$i++) { ?>
									...
									<tr id="tr-<?php echo $carts[$i]['id']; ?>">
										...
										<td>
											<?php language('phone'); ?><br><input type="text" class="form-control" name="phone" value="<?php if(isset($_COOKIE['phone'])) echo $_COOKIE['phone']; ?>" pattern="[0-9]*" title="<?php language('quantitymatch'); ?>" maxlength="11" style="background-color: #dbdbdb;" required="required" />
											<br><br>
											<?php language('address'); ?><br><textarea name="address" class="form-control" rows="3" style="background-color: #dbdbdb;" required="required"><?php if(isset($_COOKIE['address'])) echo $_COOKIE['address']; ?></textarea>
										</td>
										<td style="width: 125px;">											
											...
											<br><br>
											<input type="submit" name="paypal" value="<?php language('paypal'); ?>" formaction="paypal.php" class="btn btn-info" style="background-color: #575757; width: 125px; border: 1px solid #575757;">
											<br><br>
											<input type="submit" name="ondelivery" value="<?php language('ondelivery'); ?>" formaction="ondelivery.php" class="btn btn-info" style="background-color: #575757; width: 125px; border: 1px solid #575757;">
											<?php } else { echo '<div style="color: red;">'; language('cantbuy'); echo '</div>'; } ?>
										</td>
										<td><i class="fa fa-times fa-5x" style="width: 75px; color: #575757;" onclick="deletecart('<?php echo $carts[$i]['id'];?>','<?php echo $carts[$i]['product'];?>','<?php echo $ipaddress;?>','<?php echo $carts[$i]['quantity'];?>')"></i></td>
										</form>
									</tr>
								<?php } ?>
							</table>
