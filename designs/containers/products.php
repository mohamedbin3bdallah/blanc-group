		...
		<div class="row">
			<div class="col-md-12">
				...
				<table class="table table-bordered">
					...
					<?php if(isset($product['sizes']) && $product['sizes'] != '') { $productsizes = explode(",",$product['sizes']); ?>
					<tr>
						<td class="info"><?php language('currentsizes'); ?></td>
						<td class="warning">
							<div>
								<?php for($s=0;$s<count($productsizes);$s++) { ?>
									<div style="float: left; margin: 5px;"><?php echo $productsizes[$s]; ?></div>
								<?php } ?>
							</div>
						</td>
					</tr>
					...
					<tr>
						<td class="info"><?php language('sizes'); ?></td>
						<td class="warning">
							<select id="multiuserselect" size="3" name="sizes[]" class="form-control" title="<?php language('selectmatch'); ?>" required="required" multiple>
								<?php getSelectData('sizes'); ?>
							</select>
						</td>
					</tr>
					
					<?php if(isset($product['colors']) && $product['colors'] != '') { $productcolors = explode(",",$product['colors']); ?>
					<tr>
						<td class="info"><?php language('currentcolors'); ?></td>
						<td class="warning">
							<div>
								<?php for($co=0;$co<count($productcolors);$co++) { ?>
									<div style="float: left; height: 33px; width: 33px; margin: 5px; background-color: #<?php echo $productcolors[$co]; ?>;"></div>
								<?php } ?>
							</div>
						</td>
					</tr>
					...
					<tr>
						<td class="info"><?php language('colors'); ?></td>
						<td class="warning">
							<select id="multiuserselect" size="3" name="colors[]" class="form-control" title="<?php language('selectmatch'); ?>" required="required" multiple>
								<?php getSelectColor($lang_file); ?>
							</select>
						</td>
					</tr>
					<tr>
								...
								<div class="modal-dialog modal-lg">
									<div class="modal-content">
										...
										<div class="modal-body">
											...
											<div><h4><?php language('sizes'); ?></h4></div>
											<?php if(isset($products[$i]['sizes']) && $products[$i]['sizes'] != '') { $productsizes[$i] = explode(",",$products[$i]['sizes']); ?>
												<div>
													<?php for($s=0;$s<count($productsizes[$i]);$s++) { ?>
														<div style="float: left; margin: 5px;"><?php echo $productsizes[$i][$s]; ?></div>
													<?php } ?>
												</div>
											<?php } else language('nosizes'); ?><br><br><br>
											
											<div><h4><?php language('colors'); ?></h4></div>
											<?php if(isset($products[$i]['colors']) && $products[$i]['colors'] != '') { $productcolors[$i] = explode(",",$products[$i]['colors']); ?>
												<div>
													<?php for($co=0;$co<count($productcolors[$i]);$co++) { ?>
														<div style="float: left; height: 33px; width: 33px; margin: 5px; background-color: #<?php echo $productcolors[$i][$co]; ?>;"></div>
													<?php } ?>
												</div>
											<?php } else language('nocolors'); ?><br><br><br>
										</div>
									</div>
								</div>
							</div>
